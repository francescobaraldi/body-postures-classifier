...
decision_tree_definitivo_v1 = tree.DecisionTreeClassifier(random_state=3, max_depth=20, class_weight=None,
                                                          criterion='entropy', splitter='best')  # F1-score = 0.9851278588981345
...
importance = pd.Series(data=decision_tree_definitivo_v1.feature_importances_,
                       index=X_train_v1.columns).sort_values(ascending=False)
...
decision_tree_definitivo_v2 = tree.DecisionTreeClassifier(random_state=3, max_depth=50, class_weight='balanced',
                                                          criterion='entropy', splitter='best')  # F1-score = 0.9818006456087918
...
importance = pd.Series(data=decision_tree_definitivo_v2.feature_importances_,
                       index=X_train_v2_scaled.columns).sort_values(ascending=False)
plt.figure(figsize=(10, 10))
plt.title("Feature importance")
sns.barplot(y=importance.index, x=importance.values, palette="Blues_d", orient='h')
...
svm_definitivo_v2 = make_pipeline(pca, svc)  # F1-score = 0.5545493382132174
...
decision_tree_definitivo_v3 = RandomForestClassifier(random_state=3, max_depth=20, class_weight=None,
                                                     criterion='entropy', n_estimators=50)  # F1-score = 0.9958063054386616
...
importance = pd.Series(data=decision_tree_definitivo_v3.feature_importances_,
                       index=X_train_v1.columns).sort_values(ascending=False)
plt.figure(figsize=(10, 10))
plt.title("Feature importance")
sns.barplot(y=importance.index, x=importance.values, palette="Blues_d", orient='h')
...
print('The best value for parameter n_estimators is', clf.best_params_.get('n_estimators'),
      'since these lead to F1-score =', clf.best_score_)
svm_definitivo_v3 = BaggingClassifier(random_state=3, base_estimator=svm_definitivo_v2, verbose=1, n_estimators=100)  # F1-score = 0.6147914579142865
...
iperparametri = {'n_estimators': [10, 20]}
...
nn_definitivo_v3 = BaggingClassifier(random_state=3, base_estimator=nn_definitivo_v1, verbose=1,
                                     n_estimators=20)
nn_definitivo_v3.fit(X_train_v1, y_train_v1)
y_pred = nn_definitivo_v3.predict(X_test_v1)
execute_scores("NN v3", y_test_v1, y_pred)
